from django.test import TestCase
from django.http import HttpRequest
from django.contrib.auth.models import User
import base64
import tempfile
from django.test import TestCase, override_settings
from django.core.files.uploadedfile import InMemoryUploadedFile
from io import BytesIO

from part1.forms import SignUpForm, VolunteerForm
from part1.models import CustomUser, Volunteer


class TestSignUpForm(TestCase):
    def test_empty_form(self):
        form = SignUpForm()
        self.assertIn("full_name", form.fields)
        self.assertIn("mobileNum", form.fields)
        self.assertIn("email", form.fields)
        self.assertInHTML(
            '<input type="text" name="full_name" maxlength="100" required id="id_full_name" placeholder="Full Name">',
            '<input type="text" name="full_name" maxlength="100" required id="id_full_name" placeholder="Full Name">'
        )
        self.assertInHTML(
            '<input type="email" name="email" maxlength="254" id="id_email" placeholder="Email">',
            '<input type="email" name="email" maxlength="254" id="id_email" placeholder="Email">'
        )
        self.assertInHTML(
            '<input type="number" name="mobileNum" min="0" required id="id_mobileNum" placeholder="Mobile Number">',
            '<input type="number" name="mobileNum" min="0" required id="id_mobileNum" placeholder="Mobile Number">'
        )

    def test_SignUp_user(self):
        request = HttpRequest()
        request.POST = {
            "username": "rokib",
            "full_name": "Rokibul Hasan",
            "mobileNum": "01952578869",
            "email": "just-for-testing@testing.com",
            "password1": "dummy-insecure1!",
            "password2": "dummy-insecure1!",
        }
        form = SignUpForm(request.POST)
        form.save()
        self.assertEqual(CustomUser.objects.count(), 1)


class TestVolunteerForm(TestCase):
    def test_empty_form(self):
        form = VolunteerForm()
        self.assertIn("address", form.fields)
        self.assertIn("img", form.fields)
        self.assertInHTML(
            '<input type="file" Required name="img">',
            '<input type="file" Required name="img">'
        )
        self.assertInHTML(
            '<input type="text" Required name="address" maxlength="200" placeholder="Address">',
            '<input type="text" Required name="address" maxlength="200" placeholder="Address">'
        )

    @override_settings(MEDIA_ROOT=tempfile.gettempdir())
    def test_Volunteer(self):
        image = InMemoryUploadedFile(BytesIO(base64.b64decode(TEST_IMAGE)), field_name='tempfile', name='tempfile.png',
            content_type='image/png',
            size=len(TEST_IMAGE),
            charset='utf-8',
        )

        form = VolunteerForm({'address': "wari,Dhaka", 'image': image})
        self.assertEqual(form.is_valid(), True)


TEST_IMAGE = '''

'''.strip()