...
from part1.models import CustomUser, PickUppoints
...
class CustomUserModelTest(TestCase):
    ...
    def test_mobileNum_label(self):
        user = CustomUser.objects.get(id=1)
        field_label = user._meta.get_field('mobileNum').verbose_name
        self.assertEqual(field_label, 'mobileNum')
...
class PickUppointsModelTest(TestCase):
    ...
    def setUpTestData(cls):
        cls.point = PickUppoints.objects.create(
            Division="Dhaka",
            District="Dhaka",
            address="Aftabnagar,Rampura",
            img=InMemoryUploadedFile(BytesIO(base64.b64decode(TEST_IMAGE)), field_name='tempfile', name='tempfile.png',
